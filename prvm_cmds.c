...
// [EXT_WRATH] #438 float(entity e, vector p[, float skipmask]) getsurfacenearpoint = #438;
void VM_getsurfacenearpoint(prvm_prog_t *prog)
{
	int surfacenum, best, skipmask;
	vec3_t clipped, p;
	vec_t dist, bestdist;
	prvm_edict_t *ed;
	dp_model_t *model;
	...
	skipmask = (prog->argc > 2) ? (int)PRVM_G_FLOAT(OFS_PARM2) : 0;
